...
<template>
    <div class="playgroundRoot">
        ...
        >
            ...
            <tab-item label="Message" ref="messageTab" class="messagePanel">
                <message 
                style="overflow: hidden; height: 100%;"
                ref="message">
            </message>
...
function initEditor(vm) {
    ...
    async function doRunScriptAsync(editor, el, updateOps, message) {
        if (runScriptPromise) {
            console.log(
        ...
        try {
            await (runScriptPromise = Runner.runScript(script, appendOutput, updateOps, event));
        } catch (ex) {
            appendOutput(`\nEXCEPTION: "${ex}"`);
        } finally {
    ...
    async function doRunScript(editor, isAsync, resultEl, updateOps, message) {
        if (isScriptRunning) {
            console.log(
                "Blocked run script request as another script is already running."
            );
        ...
        if (isAsync) {
            await doRunScriptAsync(editor, resultEl, updateOps, message);
        } else {
            await doRunScriptSync(editor, resultEl, message);
        }
...
export default {
    ...
    methods: {
        ...
        getMessage() {
            return this.$refs.message.getEditor();
        },
        stopScript() {
            Runner.stopScript();
        },
    ...
    mounted() {
        const cm = this.getEditor();
        const mm = this.getMessage();
        const r = initEditor(this);
        r.tryCompileDebounced.trigger(cm);
        this.$_r = r;
        this.$nextTick(() => {
            mm.setValue(this.initialMessage)
            cm.refresh();
            cm.setValue(this.initialCode);
            cm.focus();
        });
    },
    components: { AstView, Message, Editor, SplittableTabs, TabItem },
};
</script>
